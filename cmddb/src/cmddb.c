...
int cmddb_query__(struct sqlite3 *db, const char *query, int len) {
	...
	case SQLITE_DONE:
		retval = SQLITE_OK;
		if (dbglev)
			fprintf(stderr, "Info: Query done ok!! \"%s\"\n", query);
		break;
	case SQLITE_ROW:
		if (dbglev)
	...
	default:
		if (dbglev)
			fprintf(stderr, "Error: Query faulted !! \"%s\"\n", query);
		retval = -EFAULT;
		break;
	}
...
int cmddb_db_init(struct sqlite3 *db) {
	...
	retval
			= cmddb_query(
					db,
					DB_CREATE_TABLE_CMD " " DB_TABLE_NAME_CMD_LST " " DB_TABLE_COLU_CMD_LST,
					0);
	if (retval != 0) {
...
int cmddb_db_print(void) {
	...
	len = sprintf(query,
			"SELECT cid, name, value, ttime, status, stime, retval, sent, pseq "
	...
	printf("%4s , %-20s , %-20s , %-6s , %-19s , %-19s , %-4s , %4s , %-30s\n",
			"cid", "name", "value", "status", "trigger time", "status time",
			"sent", "dep", "retval");

	while ((retval = sqlite3_step(stathandel)) == SQLITE_ROW) {
		char time_str[64];
		char stime_str[64];
		cmddb_time_print((time_t) sqlite3_column_int(stathandel, 3), time_str,
				sizeof(time_str));
		cmddb_time_print((time_t) sqlite3_column_int(stathandel, 5), stime_str,
				sizeof(stime_str));
		printf(
				"%4d , %-20s , %-20s , %6d , %-19s , %-19s , %-4d , %4d , %-30s\n",
				sqlite3_column_int(stathandel, 0), sqlite3_column_text(
						stathandel, 1), sqlite3_column_text(stathandel, 2),
				sqlite3_column_int(stathandel, 4), time_str, stime_str,
				sqlite3_column_int(stathandel, 7), sqlite3_column_int(
						stathandel, 8), sqlite3_column_text(stathandel, 6));
	}
...
int cmddb_insert(int cid, const char *name, const char *value, time_t ttime,
	...
	" VALUES ( %d, '%s', '%s', %d, %lu, %lu, 0, %d)", cid, name, value, status,
			ttime, now, pseq);
...
int cmddb_mark_remove(int cid) {
	...
	" SET sent=-1 , status = %d WHERE cid=%d AND status = %d ", CMDSTA_DELETED,
			cid, CMDSTA_QUEUED);
 ...
 */
int cmddb_status_set(int id, const char *col, int status, char* retval,
	...
	len += snprintf(query + len, sizeof(query) - len,
			"UPDATE "DB_TABLE_NAME_CMD_LST
			" SET status=%d, stime=%lu, sent=%d", status, now, 0);

	if (retval) {
		len += snprintf(query + len, sizeof(query) - len, ", retval='%s'",
				retval);
		printf("retval %s\n", retval);
	}
...
int cmddb_cmd_exec(struct cmddb_cmd *cmd, struct cmddb_desc *desc,
		void *sessdata, unsigned long rowid, time_t now) {
	...
	cmddb_status_set(rowid, "rowid", status, retval, remsent, now - (time(NULL)
			- t_start));
...
int cmddb_exec_list(struct cmddb_desc *list, void *sessdata, time_t now) {
	...
	while (ptr) {
		len += snprintf(query + len, sizeof(query) - len, "'%s'", ptr->name);
		if (ptr->next)
			len += snprintf(query + len, sizeof(query) - len, ",");
		ptr = ptr->next;
	}

	len += snprintf(query + len, sizeof(query) - len, ") ORDER BY ttime ASC");
	...
	while (sqlite3_step(stathandel) == SQLITE_ROW) {
		...
		0 /*stime*/, sqlite3_column_int(stathandel, 4), NULL/* retval*/);

		cmddb_cmd_exec(execcmd, desc, sessdata, sqlite3_column_int(stathandel,
				6), now);
		count++;
...
time_t cmddb_time_execed(const char *name, time_t now) {
	...
	len
			= snprintf(
					...
					"AND ttime<=%lu ORDER BY ttime DESC", name,
					CMDSTA_EXECUTED, now);
...
int cmddb_mark_prev(char *name, time_t ttime) {
	...
	if (dbglev)
		fprintf(stderr, "boxvcmd_mark_prev: %s, %lu\n", name, ttime);
	...
	len
			+= snprintf(query + len, sizeof(query) - len, " AND ttime < %lu",
					ttime);

	if (dbglev)
		fprintf(stderr, "boxvcmd_mark_prev: %s\n", query);
...
int cmddb_clean(const char *cond) {
	...
	while (sqlite3_step(stathandel) == SQLITE_ROW) {
		...
		len += snprintf(query + len, sizeof(query) - len, "%d",
