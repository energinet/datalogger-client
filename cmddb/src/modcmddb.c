...
int cmdvar_initdef(struct cmdvar *var) {
	...
	if (t_update) {
		...
		module_base_send_event(module_event_create(var->event_type,
				cmdvar_conv(var, buf), &var->t_update));
...
int cmdvar_cmd_handler(CMD_HND_PARAM) {
	...
	retval = module_base_send_event(module_event_create(var->event_type,
			cmdvar_conv(var, cmd->value), &var->t_update));

	if (retval) {
		char errstr[512];
		int subscnt = event_type_get_subscount(var->event_type);
		if (subscnt == -retval)
			snprintf(errstr, sizeof(errstr), "seterror in all (count %d)",
					subscnt);
		else
			snprintf(errstr, sizeof(errstr), "seterror in %d of %d", -retval,
					subscnt);
...
int module_add_cmdvar(struct module_object* this, struct cmdvar *var,
		const char **attr) {
	...
	this->descs = cmddb_desc_add(this->descs, cmddb_desc_create(name,
			cmdvar_cmd_handler, (void*) var, CMDFLA_SETTING));
	...
	this->base.event_types = event_type_add(this->base.event_types,
			var->event_type); // make it avaliable
	...
	if (event_name) {
		...
		return module_base_subscribe_event(&this->base, this->base.first,
...
struct xml_tag module_tags[] = { { "var", "module", start_cmdvar, NULL, NULL },
		{ "sum", "module", start_cmdsum, NULL, NULL }, { "", "", NULL, NULL,
				NULL } };

int module_init(struct module_base *module, const char **attr) {
	struct module_object* this = module_get_struct(module);

	return cmddb_db_open(get_attr_str_ptr(attr, "path"), 0);;
...
struct module_type
		module_type = { .name = "cmddb", .xml_tags = module_tags,
				.handlers = handlers,
