...
int socket_hndlr_reads_upd(ASOCKET_FUNC_PARAM)
{
    ...
    unsigned long mask = event_type_get_flags(asocket_cmd_param_get(rx_msg, 0),  FLAG_SHOW );
    ...
    sconn->event_list = module_base_read_all(base->first, asocket_cmd_param_get(rx_msg, 1), mask );
...
int socket_hndlr_read_get(ASOCKET_FUNC_PARAM)
{
    ...
    if(sconn->event_next){
		...
		char buf[512];
		const char *hname =  event->type->hname;
		if(!hname)
			hname = "";
		snprintf(buf,sizeof(buf), "%s.%s", event->source->name, event->type->name);
		asocket_cmd_param_add(tx_msg, buf);
		asocket_cmd_param_add(tx_msg, hname);  
		uni_data_get_txt_fvalue(event->data, buf, sizeof(buf),  type->flunit);
//	snprintf(buf,sizeof(buf), "%.2f", uni_data_get_fvalue(event->data));
		asocket_cmd_param_add(tx_msg, buf);  
		if(!type->flunit)
...
struct socket_cmd socket_cmd_list[] = {
    { "ReadsUpdate" , socket_hndlr_reads_upd, "Update read values param1: flags, param2: name "},
    { "ReadGet"     , socket_hndlr_read_get , "Get read value"},   
