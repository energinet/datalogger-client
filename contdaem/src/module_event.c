...
struct event_type *event_type_create_attr(struct module_base *base, void *obj,
		const char **attr) {
	return event_type_create(base, obj, get_attr_str_ptr(attr, "name"),
			get_attr_str_ptr(attr, "unit"), get_attr_str_ptr(attr, "text"),
			event_type_get_flags(get_attr_str_ptr(attr, "flags"), base->flags));

}

struct event_type *event_type_create_copy(struct module_base *base, void *obj,
		unsigned long flag, struct event_type *source) {
	return event_type_create(base, obj, source->name, source->unit,
			source->hname, flag);
}
...
struct event_type *event_type_create_copypart(struct module_base *base,
	...
	return event_type_create(base, obj, name, unit, text, event_type_get_flags(
			get_attr_str_ptr(attr, "flags"), base->flags));
}
...
struct event_type *event_type_list_rem(struct event_type *first,
		struct event_type *rem) {
	struct event_type *ptr = first;
...
struct event_handler *event_handler_list_add(struct event_handler *first,
...
int module_event_sprint(struct module_event* event, char *buf, int maxlen) {
	...
	assert(event->type);
	assert(event->data);
	snprintf(buf + len, maxlen - len, "%s.%s                ",
...
struct module_base* event_search_next_module__(struct module_base* current,
		const char *name) {
...
struct event_type *event_search_next(struct event_search* search) {
	...
	while (search->m_curr) {
		if (search->dbglev)
			fprintf(stderr, "evt s: m %s\n", search->m_curr->name);

		while (search->e_next) {
			struct event_type *current = search->e_next;
			if (search->dbglev)
				fprintf(stderr, "evt s: e %s\n", current->name);
			search->e_next = search->e_next->next;
			if ((current->flags & search->mask) && (current->flags & FLAG_AVAI)) {
				if (search->dbglev)
					fprintf(stderr, "OK flags %s\n", current->name);
				if ((!search->e_name) || (strcmp(search->e_name, current->name)
						== 0)) {
					if (search->dbglev)
		...
		search->m_curr = event_search_next_module__(search->m_curr->next,
				search->m_name);
