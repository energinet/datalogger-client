...
enum trigtype {
	TRIG_ALL, TRIG_HIGH, TRIG_LOW,
};

enum activtype {
	ACTIV_HIGH, ACTIV_LOW,
};

enum keytype {
	KEY_COUNT, KEY_TRIGGER,
};
 ...
 * @ref trigtype
 */
const char *trigtyps[] = { "all", "high", "low", "allactlow" };

/**
 * Input active type strings 
 * @ref activtype
 */
const char *activtyps[] = { "high", "low" };
...
struct key_obj *key_obj_create(enum keytype type, int key_code,
		enum trigtype trig, int timeout) {
...
int key_obj_value_update(struct keymod_object *module, struct key_obj *keyobj,
	...
	if (keyobj->trigger.active == ACTIV_LOW)
		keyobj->value = !value;
	else
		keyobj->value = value;
...
int key_obj_init_state(struct keymod_object *module, struct key_obj *key_obj) {
	...
	key_obj_value_update(module, key_obj, NULL, (key_obj->trigger.active
			== ACTIV_LOW) ? 1 : 0);//Todo
...
void module_send_event(struct keymod_object *module,
...
int key_obj_count_event(struct keymod_object *module, struct key_obj *keyobj,
		struct timeval *time, int pulse) {
	...
	if (keyobj->count.count && (diff_reset > module->interval)) {
		unsigned long diff = timeval_diff(&keyobj->last_pulse,
				&keyobj->last_reset);
		module_send_event(module, keyobj->event_type, &keyobj->last_pulse,
				diff, keyobj->count.count * keyobj->count.upp);
...
int key_obj_trigger_event(struct keymod_object *module, struct key_obj *keyobj,
		struct timeval *time, int hit) {
	struct uni_data *data;

	int timediff = 0;
	if (keyobj->last_pulse.tv_sec && time) {
		timediff = timeval_diff(time, &keyobj->last_pulse) / 1000;

		if (timediff < keyobj->trigger.ms_wait_chatter)
			return 0;
	}

	if (hit || keyobj->trigger.value_sent != keyobj->value) {
		printf("value %d --> sent %d\n", keyobj->trigger.value_sent,
				keyobj->value);
		if (time) {
			memcpy(&keyobj->last_pulse, time, sizeof(*time));
		} else {
		...
		data = uni_data_create_int(keyobj->value);
		data->mtime = timediff;

		module_base_send_event(module_event_create(keyobj->event_type, data,
				&keyobj->last_pulse));

		keyobj->trigger.value_sent = keyobj->value;
...
struct module_event *key_obj_eread(struct event_type *event) {
	...
	return module_event_create(event, key_obj_get_ldata(keyobj),
			&keyobj->last_pulse);
}
...
int start_setup(XML_START_PAR) {
	...
	this->timeout = SEC_UNIT * get_attr_int(attr, "timeout", this->timeout
			/ SEC_UNIT );
	this->interval = SEC_UNIT * get_attr_int(attr, "interval", this->interval
			/ SEC_UNIT );
...
int start_add_event_type(struct keymod_object* module, struct key_obj *key_obj,
		const char **attr) {

	key_obj->event_type = event_type_create_attr(&module->base,
			(void*) key_obj, attr);
	...
	module->base.event_types = event_type_add(module->base.event_types,
			key_obj->event_type);
...
int start_count(XML_START_PAR) {
	...
	enum trigtype trig = (enum trigtype) get_item(
			get_attr_str_ptr(attr, "trig"), TRIG_HIGH, trigtyps, 3);

	printf("trig is %s (%d) --> %s\n", get_attr_str_ptr(attr, "trig"), trig,
			trigtyps[trig]);

	new = key_obj_create(KEY_COUNT, get_attr_int(attr, "key_code",
			module->key_code_next++), /*TRIG_HIGH*/trig, get_attr_int(attr,
...
int start_trigger(XML_START_PAR) {
	...
	struct key_obj *new = NULL;
	enum trigtype trig = (enum trigtype) get_item(
			get_attr_str_ptr(attr, "trig"), TRIG_ALL, trigtyps, 3);

	enum activtype active = (enum activtype) get_item(get_attr_str_ptr(attr,
			"active"), ACTIV_HIGH, activtyps, 2);

	new = key_obj_create(KEY_TRIGGER, get_attr_int(attr, "key_code",
			module->key_code_next++), trig, get_attr_int(attr, "timeout",
			module->timeout));

	new->trigger.active = active;

	new->trigger.ms_wait_chatter = get_attr_int(attr, "chatter_wait", 3000);
...
struct xml_tag module_tags[] = {
		{ "input", "module", start_input, NULL, NULL }, { "count", "module",
				start_count, NULL, NULL }, { "trigger", "module",
				start_trigger, NULL, NULL }, { "setup", "module", start_setup,
...
struct module_type
		module_type = { .name = "keyinput", .xml_tags = module_tags,
				.handlers = handlers,
