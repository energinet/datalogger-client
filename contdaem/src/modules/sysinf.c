...
enum {
	SYSSTA_INIT = -1, SYSSTA_OK = 0, SYSSTA_THRH, SYSSTA_FAULT,
};
...
struct sys_info *sys_info_create(const char *typestr, const char *name,
		const char *path, int interval) {
	struct sys_info *new = NULL;
	int type = -1;
	int i;
	char sitypes[][32] = { "memfree", "vmused", "p_mrss", "p_mvsz",
			"parti_free", "file_size", "" };
...
void sys_info_send_m_event(struct module_object *module,
...
int sys_info_read(struct module_object *module, struct sys_info *sysinfo,
		struct timeval *time, int do_log) {
	int retval;
	float value;
	char ret[64];
	char file[64];
...
struct event_handler handlers[] = { { .name = "all",
...
int start_sysinfo(XML_START_PAR) {
	...
	struct sys_info *new = sys_info_create(get_attr_str_ptr(attr, "type"),
			get_attr_str_ptr(attr, "app"), get_attr_str_ptr(attr, "pidfile"),
	...
	new->event_type = event_type_create(&this->base, NULL, name, unit, text,
			this->base.flags);
...
int start_partfree(XML_START_PAR) {
	...
	const char *text_alt;
	struct module_object* this = module_get_struct(ele->parent->data);
	struct sys_info *new = sys_info_create("parti_free", get_attr_str_ptr(attr,
			"name"), get_attr_str_ptr(attr, "path"), get_attr_int(attr,
			"interval", 600));
...
int start_filesize(XML_START_PAR) {
	...
	const char *text_alt;
	struct module_object* this = module_get_struct(ele->parent->data);
	struct sys_info *new = sys_info_create("file_size", get_attr_str_ptr(attr,
			"name"), get_attr_str_ptr(attr, "path"), get_attr_int(attr,
			"interval", 600));
...
struct xml_tag module_tags[] = {
		{ "info", "module", start_sysinfo, NULL, NULL }, { "filesize",
				"module", start_filesize, NULL, NULL }, { "partfree", "module",
				start_partfree, NULL, NULL }, { "", "", NULL, NULL, NULL } };
...
int sys_info_state_upd(struct module_object *module) {
	...
	if (module->fault_state != new_state) {
		...
		module_base_send_event(module_event_create(module->state_event, &data,
				&time));
...
int module_init(struct module_base *module, const char **attr) {
	...
	this->state_event = event_type_create(module, NULL, "state", "",
			"Application State", DEFAULT_FFEVT );
	...
	module->event_types
			= event_type_add(module->event_types, this->state_event); // make it avaliable
...
void* module_loop(void *parm) {
	...
	while (base->run) {
		...
		while (sysinf) {
			sys_info_read(this, sysinf, &tv, (tv.tv_sec % sysinf->interval)
					== 0);
			sysinf = sysinf->next;
		}
...
struct module_type
		module_type = { .name = "sysinf", .xml_tags = module_tags,
				.handlers = handlers,
