...
struct query_resitem *query_resitem_add(struct query_resitem *first, struct query_resitem *new){
...
int db_sqlite_cmd(struct db_sqlite_object *db_obj, const char *db_statement, struct query_result *result)
{
    int retval = -EFAULT;
    char *zErrMsg = NULL;
    ...
    retval = sqlite3_exec(db_obj->db, db_statement, db_callback, result, &zErrMsg);
...
int db_sqlite_init_tables(struct db_sqlite_object *db_obj)
{
    ...
    retval = db_sqlite_cmd(db_obj, DB_CREATE_TABLE_CMD " " DB_TABLE_NAME_EVENT_LOG " " DB_TABLE_COLU_EVENT_LOG, NULL);
    if (retval < 0){
        PRINT_ERR("error creating table %s", DB_TABLE_NAME_EVENT_LOG);
        return retval;
    }

    retval = db_sqlite_cmd(db_obj, DB_CREATE_TABLE_CMD " " DB_TABLE_NAME_EVENT_TYPE " " DB_TABLE_COLU_EVENT_TYPE, NULL);
    if (retval < 0){
        PRINT_ERR("error creating table %s", DB_TABLE_NAME_EVENT_TYPE);
        return retval;
    }
    
    retval = db_sqlite_cmd(db_obj, DB_CREATE_TABLE_CMD " " DB_TABLE_NAME_TIME_INDEX " " DB_TABLE_COLU_TIME_INDEX, NULL);
    if (retval < 0){
...
//int db_sqlite_insert_event(struct db_sqlite_object *db_obj, struct module_event *event);


int db_sqlite_open(struct db_sqlite_object *db_obj, const char *path, int verbose)
{
    int retval = -EFAULT;
...
struct query_resitem *db_sqlite_get_list(struct db_sqlite_object *db_obj, const char *query)
{
...
int db_sqlite_get_first_int(struct db_sqlite_object *db_obj, const char *query, int *result)
{
    struct query_result qresult;
    int retval = 0;
...
int db_sqlite_update_event_type(struct db_sqlite_object *db_obj, const char *name, const char *unit, const char *hname)
{
    char query[512];
    ...
    if(eid<0)
        snprintf(query, 512, "INSERT INTO %s (ename, unit, descript) VALUES ('%s', '%s' , '%s')" ,
...
int db_sqlite_create_event_type(struct db_sqlite_object *db_obj, const char *name, const char *unit, const char *hname)
{
    char query[512];
    snprintf(query, 512, "INSERT INTO %s (ename, unit, descript) VALUES ('%s', '%s' , '%s')" ,
             DB_TABLE_NAME_EVENT_TYPE, name, unit, hname);
...
int db_sqlite_get_event_id(struct db_sqlite_object *db_obj, const char *ename)
{
    ...
    int eid = -1;
    int retval = 0;
    snprintf(query, 512, "SELECT eid FROM %s WHERE ename ='%s'" , DB_TABLE_NAME_EVENT_TYPE, ename);
...
/* int db_sqlite_update_index(struct db_sqlite_object *db_obj, char *time) */
/* { */
/*     char query[512]; */
/*     int eid = -1; */
/*     int retval = 0; */
/*     snprintf(query, 512, "SELECT time FROM %s WHERE time > (%s-%d)" , DB_TABLE_NAME_EVENT_INDEX, ename); */

/*     retval = db_sqlite_get_first_int(db_obj, query, &eid); */

/*     if(retval == 0){ */
/*         PRINT_DBG(1, "ename == %s does not exist", ename); */
/*         return -1; */
/*     } else if( retval < 0) { */
/*         PRINT_ERR("db error getting eid for %s", ename); */
/*         return -2; */
...
/*     PRINT_DBG(1, "%s has eid %d", ename, eid); */
