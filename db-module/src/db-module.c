...
int module_write_log(struct module_object* this, struct module_event* event) {
	...
	if (eid == -1) {
		logdb_etype_add(this->db, ename, event->type->hname, event->type->unit,
				"m");
		eid = logdb_etype_get_eid(this->db, ename);
	} else if (eid < -1)
		return -1;
...
struct xml_tag module_tags[] = {
		{ "sqlite", "module", start_sqfile, NULL, NULL }, { "log", "module",
				start_log, NULL, NULL }, { "", "", NULL, NULL, NULL } };
...
struct module_type module_type = { .name = "dblogger", .mtype = MTYPE_DATALOG,
		.subtype = SUBTYPE_NONE, .xml_tags = module_tags, .handlers = handlers,
