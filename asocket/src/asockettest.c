...
int vpp_cmd_boxinfo(ASOCKET_FUNC_PARAM)
{
    ...
    asocket_con_snd_vp(sk_con, "BoxInfo", 3 , vpp_box->id, vpp_box->type, vpp_box->verion);
...
int vpp_cmd_ioset(ASOCKET_FUNC_PARAM)
{
    ...
    asocket_con_snd_vp(sk_con, "IoUpdate", 2 , name_str, (vpp_box->output[number-1])?"on":"off");
...
int vpp_client(const char *ip, int port)
{
    ...
    struct vpp_box vpp_box = {"00:15:8C:00:18:26", "demo", "0.01" , {0,0}, {0}};

    struct asocket_cmd vpp_cmd_list[] = {
        { "BoxInfoGet"   ,    vpp_cmd_boxinfo, NULL},
        { "IoSet",   vpp_cmd_ioset , NULL}, 
        { "StatusGet",   vpp_cmd_statusget , NULL}, 
        { "ReadingGet",   vpp_cmd_readingget , NULL }, 
        { "Close",  NULL , NULL}, 
        { NULL,   NULL }
    };
