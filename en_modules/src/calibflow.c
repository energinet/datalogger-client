...
int calibflow_status_print(struct module_object* this, char*buffer, size_t size) {
	int len = 0;
	time_t duration = 0;
	char *states[] = { "Stoppet", "Vand løber", "Klar", "Måler", "Færdig",
			"Sendt", "Fejl" };
	...
	case CALIB_RUNNING:
		...
		len += snprintf(buffer + len, size - len,
				": %2.2f l  / %d sek =  %2.2f l/min", this->count, duration,
...
void calibflow_send(struct module_object* this) {
	module_base_send_event(module_event_create(this->status_event,
			calibflow_status_data(this), NULL));
...
int start_input(XML_START_PAR) {
	...
	flow_event_handler = event_handler_create("flowrcv", handler_flow, base,
			NULL);
	base->event_handlers = event_handler_list_add(base->event_handlers,
			cmd_event_handler);

	/* Create an event type for status notifications */
	this->status_event = event_type_create(base, NULL, "status", ".",
			"Flowkalibrering", event_type_get_flags(get_attr_str_ptr(attr,
					"flags"), base->flags));
	...
	if ((retval = module_base_subscribe_event(base, base->first, event_cmd,
			FLAG_ALL, cmd_event_handler, attr)) < 0) {
		PRINT_ERR("Calibflow could not subscribe to %s", event_cmd);
		return retval;
	}

	if ((retval = module_base_subscribe_event(base, base->first, event_flow,
			FLAG_ALL, flow_event_handler, attr)) < 0) {
...
struct xml_tag module_tags[] = {
		{ "inputs", "module", start_input, NULL, NULL }, { "", "", NULL, NULL,
				NULL } };
...
struct event_handler handlers[] = {
		{ .name = "rcv", .function = handler_flow }, { .name = "" } };

struct module_type
		module_type = { .name = "calibflow", .mtype = MTYPE_CONTROL,
